...
def _resolve_data_path(data_path, data_type):
    if not isinstance(data_path, str) or not data_path.strip():
        raise ValueError("data_path must be a non-empty path string.")
    ...
    candidate_paths = [
        os.path.join(data_path, f"hw5_data_{normalized_data_type}.parquet"),
        os.path.join(data_path, f"{normalized_data_type}.parquet"),
        os.path.join(data_path, f"data_{normalized_data_type}.parquet"),
    ]

    # Validation is often generated by splitting train data at runtime.
    if normalized_data_type == "valid":
        candidate_paths.extend(
            [
                os.path.join(data_path, "hw5_data_train.parquet"),
                os.path.join(data_path, "train.parquet"),
                os.path.join(data_path, "data_train.parquet"),
            ]
        )
    ...
    raise FileNotFoundError(
        "Could not resolve parquet path from data_path='{}' for split='{}'. "
        "Tried: {}".format(data_path, normalized_data_type, candidate_paths)
    )
...
def _warn_if_checkpoint_too_small(path, min_bytes=1_000_000):
    if not os.path.exists(path):
        print(f"Warning: expected checkpoint '{path}' was not found.")
        return

    size = os.path.getsize(path)
    print(f"Saved checkpoint '{path}' ({size} bytes)")
    if size < min_bytes:
        print(
            f"Warning: checkpoint '{path}' is unusually small. "
            "This usually means no model parameters were saved."
        )
...
def main():
    ...
    # Usage:
    # python3 hw5_model.py [lstm|transformer] [optional_data_path]
    # If no data path is provided, use a local default in the current directory.
    ...
    data_path = sys.argv[2] if len(sys.argv) > 2 else "hw5_data_train.parquet"

    if model_type == "lstm":
        print("\n--- Training LSTM ---")
        train_loader, vocab = load_and_preprocess_data(
            data_path,
            data_type="train",
            model_type="lstm",
        ...
        # Use default architecture so saved checkpoint matches autograder load shape.
        ...
        for epoch in range(lstm_epochs):
            ...
            if valid_loss < best_valid_loss:
                ...
                _warn_if_checkpoint_too_small("lstm.pt")
        ...
        # Keep default architecture for checkpoint compatibility.
        ...
        for epoch in range(N_EPOCHS):
            ...
            if valid_acc > best_valid_acc:
                ...
                _warn_if_checkpoint_too_small("transformer.pt")
