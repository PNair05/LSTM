...
def _resolve_data_path(data_path, data_type):
    ...
    normalized_data_type = split_alias.get(str(data_type).strip().lower(), str(data_type).strip().lower())

    if data_path.endswith(".parquet"):
        return data_path

    candidate_paths = [
        f"{data_path.rstrip('/')}/hw5_data_{normalized_data_type}.parquet",
        f"{data_path.rstrip('/')}/{normalized_data_type}.parquet",
        f"{data_path.rstrip('/')}/data_{normalized_data_type}.parquet",
    ]

    # Validation is often generated by splitting the train parquet at runtime.
    if normalized_data_type in {"valid", "val", "validation"}:
        candidate_paths.extend(
            [
                f"{data_path.rstrip('/')}/hw5_data_train.parquet",
                f"{data_path.rstrip('/')}/train.parquet",
                f"{data_path.rstrip('/')}/data_train.parquet",
            ]
        )
...
def load_and_preprocess_data(data_path, data_type="train", model_type="lstm", shared_vocab=None):
    ...
    parquet_path = _resolve_data_path(data_path, normalized_data_type)
    ...
    split_df = dataframe
    if normalized_data_type in {"train", "valid"} and "label" in dataframe.columns:
        try:
            ...
            # Fall back to non-stratified split for tiny/imbalanced edge cases.
            train_df, valid_df = train_test_split(
                dataframe,
                train_size=TRAIN_SPLIT,
                random_state=SEED,
    ...
    data_loader = DataLoader(
        ...
        shuffle=normalized_data_type == "train",
        num_workers=0,
    )
...
def train(model, iterator, optimizer, criterion, device, model_type="lstm", label_smoothing=0.0):
    ...
    for batch in tqdm(iterator, desc=f"Training {model_type}", leave=False):
        ...
        if label_smoothing > 0.0:
            # Smooth binary targets: 1 -> (1 - smooth/2), 0 -> smooth/2
...
def main():
    ...
    raw_model_type = sys.argv[1] if len(sys.argv) > 1 else "lstm"
    if str(raw_model_type).strip().lower() not in {"lstm", "transformer", "transformerencoder", "transformer_encoder"}:
    ...
    if model_type == "transformer":
        ...
        def lr_lambda(epoch):
            if epoch < TRANSFORMER_WARMUP_EPOCHS:
                return float(epoch + 1) / float(max(TRANSFORMER_WARMUP_EPOCHS, 1))
            progress = (epoch - TRANSFORMER_WARMUP_EPOCHS) / max(1, TRANSFORMER_EPOCHS - TRANSFORMER_WARMUP_EPOCHS)
            return max(1e-6 / TRANSFORMER_LEARNING_RATE, 0.5 * (1.0 + math.cos(math.pi * progress)))
    ...
    for epoch in range(num_epochs):
        train_loss, train_accuracy = train(
            ...
            device,
            model_type=model_type,
            label_smoothing=TRANSFORMER_LABEL_SMOOTHING if model_type == "transformer" else 0.0,
        )
